    ...
    with open(path, "r") as fid:
        while True:
            ...
            if len(line) > 0 and line[0] != "#":
                ...
                image_name = int(elems[9].split('_')[0])
...
def calc_score(inputs, images, points3d, extrinsic, args):
    ...
    cam_center_i = -np.matmul(extrinsic[images[i].camera_id][:3, :3].transpose(), extrinsic[images[i].camera_id][:3, 3:4])[:, 0]
    cam_center_j = -np.matmul(extrinsic[images[j].camera_id][:3, :3].transpose(), extrinsic[images[j].camera_id][:3, 3:4])[:, 0]
    score = 0
    angles = []
...
def processing_single_scene(args):
    ...
    new_images = {}
    for i, image_name in enumerate(sorted(images.keys())):
        new_images[i] = images[image_name]
    images = new_images
    ...
    for i, image in images.items():
        ...
        for p3d_id in images[i].point3D_ids:
            if p3d_id == -1:
                continue
            transformed = np.matmul(extrinsic[image.camera_id], [points3d[p3d_id].xyz[0], points3d[p3d_id].xyz[1], points3d[p3d_id].xyz[2], 1])
    ...
    for i, image in images.items():
        with open(os.path.join(cam_dir, '%08d_cam.txt' % i), 'w') as f:
            ...
            for j in range(4):
                for k in range(4):
                    f.write(str(extrinsic[image.camera_id][j, k]) + ' ')
                f.write('\n')
            f.write('\nintrinsic\n')
            for j in range(3):
                for k in range(3):
                    f.write(str(intrinsic[image.camera_id][j, k]) + ' ')
                f.write('\n')
    ...
    for i in range(num_images):
        # img_path = os.path.join(image_dir, str(images[i].name) + '_p.png')
        img_path = os.path.join(image_dir, str(images[i].name))
        if not img_path.endswith(".jpg"):
            img = cv2.imread(img_path)
            cv2.imwrite(os.path.join(image_converted_dir, '%04d.jpg' % i), img)
        else:
            shutil.copyfile(os.path.join(image_dir, str(images[i].name)), os.path.join(image_converted_dir, '%04d.jpg' % i))
