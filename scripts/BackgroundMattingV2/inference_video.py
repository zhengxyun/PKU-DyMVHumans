...
parser.add_argument('--output-format', type=str, default='image_sequences', choices=['video', 'image_sequences'])
parser.add_argument('--tgt_bgr_col', type=str, default='black', choices=['green', 'white', 'black'])
...
class ImageSequenceWriter:
    ...
    def _add_batch(self, frames, index):
        ...
        for i in range(frames.shape[0]):
            ...
            frame.save(os.path.join(self.path, str(index + i).zfill(6) + '.' + self.extension))
...
if args.output_format == 'video':
    ...
    if 'pha' in args.output_types:
        pha_writer = ImageSequenceWriter(os.path.join(args.output_dir, 'pha'), 'png')
    if 'fgr' in args.output_types:
...
with torch.no_grad():
    for input_batch in tqdm(DataLoader(dataset, batch_size=1, pin_memory=True)):
        if args.video_target_bgr:
            ...
            src, bgr = input_batch
            if args.tgt_bgr_col == "green":
                # with green background
                tgt_bgr = torch.tensor([120/255, 255/255, 155/255], device=device).view(1, 3, 1, 1)
            elif args.tgt_bgr_col == "white":
                # 白色RGB(255，255，255）
                tgt_bgr = torch.tensor([255/255, 255/255, 255/255], device=device).view(1, 3, 1, 1)
            elif args.tgt_bgr_col == "black":
                # 黑色rgb(0,0,0)
                tgt_bgr = torch.tensor([0/255, 0/255, 0/255], device=device).view(1, 3, 1, 1)
