...
def generate(dataset_name, dataset_fme, base_par_dir, copy_image=True, size_h=1080, size_w=1920, is_downsample=False, downsample_scale=1, fixed_camera=True, wrong_camera=[]):
    assert is_downsample == False, "Not implemented"
    print("dataset_name:", dataset_name)
    base_dir = os.path.join(base_par_dir, dataset_name,"data_NeuS", dataset_fme)
    images_dir = f'{base_par_dir}/{dataset_name}/data_NeuS2/images/{dataset_fme}'
    output_dir = f'{base_par_dir}/{dataset_name}/data_NeuS2'
    # output_dir = os.path.join(args.output_folder, dataset_name)
    ...
    image_name = 'image'
    mask_name = 'mask'
    #test_view = [8, 13, 16, 21, 26, 32, 40, 56]
    test_view = [0, 10, 20, 30, 40, 50]
    ...
    if copy_image:
        new_image_dir = join(images_dir, "images")
    ...
    # base_rgb_dir = "images"
    base_rgb_dir =f'images/{dataset_fme}'
    ...
    for frame_i in range(1):
        ...
        file_dir = join(train_dir, f'transform_%s.json' % (dataset_fme))
    ...
    for frame_i in range(1):
        ...
        test_dir = join(output_dir, f'test')
        if not os.path.exists(test_dir):
            os.makedirs(test_dir)
        file_dir = join(test_dir, f'transform_%s.json' % (dataset_fme))
        with open(file_dir,'w') as f:
    ...
    for frame_i in range(1):
        ...
        # file_dir = join(output_dir, f'transform.json')
        file_dir = join(output_dir, f'transform_%s.json' % (dataset_fme))
        with open(file_dir,'w') as f:
            json.dump(output_test, f, indent=4)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--base_par_dir', type=str, default='/amax/zxyun/FreeViewSynthesis/DyMulHumans/datasets/part1')
    parser.add_argument('--dataset_all', action="store_true")
    parser.add_argument('--dataset_name', type=str, default='1080_Dance_Dunhuang_Single_f12')
    parser.add_argument('--dataset_fme', type=str, default='000000')
    parser.add_argument("--copy_image", action="store_true")
    # parser.add_argument('--size_h', type=int, default=1080, help='image high')
    # parser.add_argument('--size_w', type=int, default=1920, help='image high')
    args = parser.parse_args()

    if '4K' in args.dataset_name:
        size_h = 2160
        size_w = 3840
    else:
        size_h = 1080
    ...
    if args.dataset_all:
        for dataset_name in os.listdir(args.base_par_dir):
            print("dataset_name2:", dataset_name)
            generate(dataset_name, args.dataset_fme, args.base_par_dir, args.copy_image, size_h, size_w)
    else:
        generate(args.dataset_name, args.dataset_fme, args.base_par_dir, args.copy_image, size_h, size_w)
