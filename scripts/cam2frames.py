...
def cam2frames(args):
    ...
    for i in range(args.fme_st, args.fme_end+1, args.fme_itr):
        ...
        com_out_dir = os.path.join(out_dir, "com")
        ...
        os.makedirs(com_out_dir, exist_ok=True)
        ...
        for j in range(args.cam_st,cam_end):
            ...
            com_pth = os.path.join(img_path, "cam_%s" % (j), "com", "%06d.png" % i)

            img = Image.open(img_pth)
            # mask = Image.open(mask_pth)
            pha = Image.open(pha_pth)
            com = Image.open(com_pth)
            ...
            com_out_pth = os.path.join(com_out_dir, "image_c_%03d_f_%06d.png" % (j, i))
            ...
            com.save(com_out_pth)


            # # ??????????:
            # # ????(0,0,0) ????(255, 255, 255) ????(120, 255, 155)
            # if '4K' in args.scene_nm:
            #     img_size = (3840, 2160)
            # else:
            #     img_size = (1920, 1080)
